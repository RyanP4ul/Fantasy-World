...
export async function GET() {
  const res = await db.query.quests_required_items.findMany({
    orderBy: desc(quests_required_items.id),
  });
...
export async function POST(req: Request) {
  ...
  try {
    const res = await db.insert(quests_required_items).values(data);
