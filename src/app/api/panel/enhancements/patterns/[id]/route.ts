...
export async function PUT(
  ...
  try {
    ...
    const old = await db.query.enhancements_patterns.findFirst({
      where: eq(enhancements_patterns.id, Number(data.oldId)),
    });
    ...
    await db
      .transaction(async (tx) => {
        await tx
          .update(enhancements_patterns)
          .set(data)
          .where(eq(enhancements_patterns.id, Number(id)));
      })
      .catch(() => {
...
export async function DELETE(
  ...
  try {
    await db
      .transaction(async (tx) => {
        await tx.delete(enhancements_patterns).where(eq(enhancements_patterns.id, Number(id)));
      })
      .catch(() => {
        return Response.json("Error transaction", { status: 500 });
      });
